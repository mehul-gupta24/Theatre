...
hover:-translate-y-1 transition duration-300 w-62">

      <img onClick={()=> {navigate(`/movies/${movie._id}`); scrollTo(0, 0)}}
      src={movie.backdrop_path} 
      alt=""
      className="rounded-lg h-52 w-full object-cover object-bottom cursor-pointer" />

      <p className="font-semibold mt-2 truncate">{movie.title}</p>

      <p className="text-sm text-gray-400 mt-2">{new Date(movie.release_date).getFullYear()} . {movie.genres.slice(0,2).map((g)=>g.name).join(" | ")} . {timeFormat(movie.runtime)}</p>
