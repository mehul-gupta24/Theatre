...
const AddShows = () => {
  ...
  const [selectedMovie, setSelectedMovie] = useState(null) //imp
  const [dateTimeSelection, setDateTimeSelection] = useState({}) //imp
  const [dateTimeInput, setDateTimeInput] = useState("")
  const [showPrice, setShowPrice] = useState(""); //imp
  ...
  const fetchNowPlayingMovies = async() => {
    // setNowPlayingMovies(dummyShowsData) //this was dummy data is assets.js, but now i want real data from api endpoint getNowPlaying movies from the TMDB which has been done in backend showController.js file, will take from APpContext file which is here in frontend
    try {
         ...
         <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {
            nowPlayingMovies.map((movie)=>(
              <div onClick={() => setSelectedMovie(movie.id)} key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-2 transition duration-300`}>
                <div className='relative rounded-lg overflow-hidden'>
                  ...
                  <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0'>
                    ...
                    <p className='text-gray-300'>{numToThousand(movie.vote_count)} Votes</p>
                  </div>
                </div>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                  {times.map((time)=>(
                    <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                      <span>{time}</span>
                      <DeleteIcon onClick={()=> handleRemoveTime(date, time)} width={15} className='ml-2 text-red-300 hover:text-red-700 cursor-pointer' />
                    </div>
                  ))}
                </div>
              </li>
