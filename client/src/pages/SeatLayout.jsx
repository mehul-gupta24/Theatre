...
const SeatLayout = () => {
  ...
  const togglingTimeLogic = (item) => {
    if(!selectedTime || selectedTime.time != item.time ){
      //means empty
    ...
    setSelectedSeats([])
    // means same click , so make it null
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-6 lg:px-40 py-30'>
      ...
      <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        <p className='text-lg font-semi-bold px-6 '>Available timings</p>
        <div className='mt-5 space-y-1'>
          {
            show.dateTime[date].map((item)=>(
              <div onClick={() => togglingTimeLogic(item)} key={item.time} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ? ("bg-primary text-white") : ("hover:bg-primary/20")} `}>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        <div className='flex flex-col items-center mt-10 text-x text-gray-300'>
